...
from db_model.retrievers.reports import get_reports
from db_model.retrievers.reports import get_reports_by_date
from db_model.retrievers.reports import get_reports_cross_table
...
def plot_reports() -> Figure:
    ...
    agg = df.groupby(['task_description', 'task_family'], dropna=False)[
        'nb_request'].sum().reset_index()
    fig = px.bar(
...
def plot_reports_cross_table() -> Figure:
    """
    Bubble scatter across tables: total_data_quantity vs
    total_power_consumption, size=total_gpu_memory, color=task_label.
    """
