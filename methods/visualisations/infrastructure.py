...
def plot_infrastructure() -> go.Figure:
    """
    Horizontal bar subplots (one per component_type):
    manufacturer vs nb_component, colored by memory_size.
    """
    df = get_infrastructure()
    grouped = df.groupby(['component_type', 'manufacturer', 'memory_size'],
                         dropna=False)['nb_component'].sum().reset_index()
    ...
    for row, ctype in enumerate(types, start=1):
        ...
        fig.add_trace(
            go.Bar(
                ...
                orientation='h',
                marker=dict(color=sub['memory_size'], colorscale='Viridis',
                            showscale=(row == len(types))),
                name=ctype,
                showlegend=False,
            ),
            row=row, col=1,
