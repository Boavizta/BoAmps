...
class BoAmpsFlattener:
    ...
    def __init__(self, obj=None):
        ...
        if type(obj) == dict:
            ...
            self.__raw = [ dict(flatdict.FlatterDict(elt, delimiter=SEPARATOR)) for elt in obj ]
        else:
    ...
    def get_denumerated(self) -> pd.DataFrame:
        ...
        for i in range(len(self.__raw)):
            ...
            if len(keys_denumerated_unique) < len(keys_denumerated):
                ...
                continue
            self.__denumerated.append(dict(zip(keys_denumerated, self.__raw[i].values())))
        print('Processed correctly {} lines out of {}'.format(str(len(self.__denumerated)), str(len(self.__raw))))
        for err in errors:
